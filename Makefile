...
docs:
	...
	for document in docs/*.md; do\
    	...
    	docker run -v "$(PWD):/data:z" pandoc/latex "$${document}" --pdf-engine=xelatex -o "out/$${document}.pdf" & docker run -v "$(PWD):/data:z" pandoc/latex "$${document}" --toc --self-contained -t html5 -o "out/$${document}.html" & docker run -v "$(PWD):/data:z" pandoc/latex "$${document}" --self-contained -t slidy --slide-level 3 -o "out/$${document}.slides.html" & docker run -v "$(PWD):/data:z" pandoc/latex "$${document}" -t beamer --slide-level 3 --pdf-engine=xelatex -o "out/$${document}.slides.pdf";\
